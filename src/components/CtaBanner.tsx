...
export default function CtaBanner({ onOpenContact, onSendEmail }: CtaBannerProps) {
  ...
  const rightBtnsRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      // Banner card entrance
      ...
      gsap.from(rightBtnsRef.current, {
        scrollTrigger: {
          trigger: bannerRef.current,
          start: "top 85%",
        },
  ...
  return (
    <section
      id="contact"
      ref={bannerRef}
      className="py-12 px-4 sm:px-8 max-w-7xl mx-auto bg-[#f5f4ed] overflow-hidden"
    >
      <div className="relative w-full rounded-2xl sm:rounded-3xl overflow-hidden border-2.5 border-zinc-900 shadow-[6px_6px_0px_#1e1e1e] min-h-[360px] sm:min-h-[420px] flex items-center justify-center">
        ...
        <div
          ref={cardRef}
          className="relative z-10 mx-4 max-w-lg w-full text-center bg-[#FAF8F3]/92 backdrop-blur-xs p-6 sm:p-8 rounded-2xl border-2 border-zinc-900 shadow-[4px_4px_0px_#1e1e1e]"
        >
          <div ref={leftTextRef}>
